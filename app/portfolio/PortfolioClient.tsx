...
import { motion, AnimatePresence } from 'framer-motion';
import { 
  Github, Mail, Phone, GraduationCap, CheckCircle2, X, 
  MessageCircle, Linkedin, Terminal
} from 'lucide-react';
import { 
  MainProfile, Contract, Hobby, Education, 
  SoftSkill, Technical, Project 
...
const TechIcon = ({ logo, name }: { logo?: string, name: string }) => {
  return (
    <div className="relative w-12 h-12 mb-2 group-hover:scale-110 transition-transform duration-500 flex items-center justify-center">
      {logo ? (
        <Image 
          src={logo} 
          alt={name} 
        ...
        /* กรณีไม่มีรูปใน DB ให้โชว์ Icon พื้นฐาน หรือตัวอักษรตัวแรก */
        <div className="w-full h-full bg-sky-50 rounded-lg flex items-center justify-center">
          <span className="text-sky-600 font-black text-xl">{name.charAt(0)}</span>
        </div>
      )}
    </div>
  );
...
export default function PortfolioClient({ 
  ...
  const categorizedTech = technicals.reduce((acc, tech) => {
    const catName = tech.category?.name || "Other";
    if (!acc[catName]) acc[catName] = [];
    acc[catName].push(tech);
    return acc;
  }, {} as Record<string, Technical[]>);

  useEffect(() => {
    const handleScroll = () => {
      const sections = ['home', 'education', 'skills', 'hobbies', 'projects'];
  ...
  return (
    <div className="bg-[#fcfcfc] text-[#121212] selection:bg-black selection:text-white">
      
      <nav className="fixed top-0 left-0 w-full z-[100] px-8 py-4 bg-white/40 backdrop-blur-md border-b border-white/20 flex justify-between items-center transition-all duration-300">
        <div 
            className="font-black text-xl tracking-tighter cursor-pointer text-black hover:scale-105 transition-transform" 
            onClick={() => window.scrollTo({top:0, behavior:'smooth'})}
        >
            {profile?.firstname?.toUpperCase() || "MY PORTFOLIO"}
        </div>

        <div className="hidden md:flex gap-10 text-[14px] font-black tracking-[0.3em] uppercase">
            {['home', 'education', 'skills', 'hobbies','projects'].map((s) => (
            <button 
                key={s} 
                onClick={() => document.getElementById(s)?.scrollIntoView({behavior:'smooth'})} 
                className={`cursor-pointer transition-all duration-300 relative group ${activeSection === s ? 'text-sky-600' : 'text-black/60 hover:text-black'}`}
            >
                {s}
                <span className={`absolute -bottom-1 left-0 h-[2px] bg-sky-500 transition-all duration-300 ${activeSection === s ? 'w-full' : 'w-0 group-hover:w-full'}`} />
            </button>
            ))}
        </div>
        ...
        <Link 
            href="/login" 
            className="bg-black text-white px-6 py-2 rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-sky-600 hover:shadow-lg hover:shadow-sky-200 transition-all duration-300"
        >
            Login
        </Link>
      </nav>

      <section id="home" className="min-h-screen flex items-center px-6 md:px-12 pt-28 bg-[#fcfcfc] overflow-hidden">
        <div className="max-w-7xl mx-auto w-full relative">
            <div className="absolute -top-20 -left-20 text-[20rem] font-black text-black/[0.02] select-none pointer-events-none uppercase">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                <div className="lg:col-span-7 z-10 order-2 lg:order-1">
                    ...
                    >
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <span className="text-xs font-black uppercase tracking-[0.5em] text-sky-600">What About Me ?</span>
                        </div>

                        <h1 className="text-[5rem] md:text-[8.5rem] font-black leading-[0.85] tracking-tighter text-zinc-900 mb-10">
                            {profile?.firstname}
                            <span className="block text-transparent" style={{ WebkitTextStroke: '1.5px #18181b', opacity: 0.3 }}>
                                {profile?.lastname}
                            </span>
                        </h1>

                        <div className="max-w-3xl p-8 bg-white border border-zinc-100 shadow-[20px_20px_60px_rgba(0,0,0,0.05)] rounded-2xl relative">
                            <div className="absolute -top-4 -left-4 w-12 h-12 bg-sky-500 rounded-xl flex items-center justify-center text-white shadow-lg shadow-sky-200">
                                <span className="font-black text-xl">“</span>
                            </div>
                            <p className="text-lg md:text-xl font-medium leading-relaxed text-zinc-500 italic">
                                {profile?.description}
                            </p>
                ...
                <div className="lg:col-span-5 order-1 lg:order-2 relative">
                    <motion.div 
                        ...
                        className="relative"
                    >
                        <div className="relative aspect-[4/5] w-full max-w-[420px] mx-auto group">
                            <div className="absolute top-8 -right-8 w-full h-full border-2 border-sky-500 rounded-[3rem] -z-10 group-hover:top-4 group-hover:-right-4 transition-all duration-500" />
                            <div className="w-full h-full rounded-[3rem] overflow-hidden shadow-2xl relative bg-zinc-200">
                                {profile?.image_url && (
                                    <Image 
                            ...
                            <motion.div 
                                ...
                                className="absolute -bottom-6 -right-6 md:-right-12 bg-white p-6 rounded-3xl shadow-2xl border border-zinc-50 z-20"
                            >
                                <div className="text-center">
                                    <div className="text-2xl font-black text-sky-500 leading-none uppercase tracking-tighter">Software</div>
                                    <div className="text-2xl font-black text-zinc-900 leading-none uppercase tracking-tighter">Developer</div>
                                </div>
                            </motion.div>
                        </div>
                    </motion.div>
      ...
      <section id="education" className="py-24 px-8 bg-[#f4f4f4]">
        <div className="max-w-6xl mx-auto">
            <div className="flex flex-col mb-16">
                <div className="flex items-center gap-3 mb-3">
                    <div className="h-px w-8 bg-sky-500" />
                    <span className="text-[10px] font-black uppercase tracking-[0.4em] text-sky-600">Background</span>
                </div>
                <h2 className="text-5xl md:text-6xl font-black uppercase tracking-tighter text-zinc-900">Education</h2>
            </div>
            ...
            <div className="space-y-20">
                {educations.map((edu, idx) => (
                    <motion.div 
                        ...
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        className={`group relative flex flex-col ${idx % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-8 md:gap-16 items-center`}
                    >
                        <div className="w-full lg:w-[40%] aspect-video relative rounded-3xl overflow-hidden shadow-xl bg-white">
                            {edu.logo_url ? (
                                <Image src={edu.logo_url} alt={edu.name} fill className="object-cover transition-transform duration-1000 group-hover:scale-110" />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center bg-gray-200">
                                    <GraduationCap size={40} className="opacity-10" />
                                </div>
                            )}
                            <div className="absolute bottom-4 left-4 bg-black/80 backdrop-blur-md px-4 py-1.5 rounded-xl">
                                <span className="text-white font-bold text-[10px] tracking-widest uppercase">
                                    {edu.starttime ? new Date(String(edu.starttime)).getFullYear() : 'N/A'} — {edu.endtime ? new Date(String(edu.endtime)).getFullYear() : 'PRESENT'}
                                </span>
                            </div>
                        </div>

                        <div className={`w-full lg:w-[60%] flex flex-col ${idx % 2 === 0 ? 'lg:pl-4' : 'lg:pr-4'}`}>
                            <h3 className="text-3xl md:text-4xl font-black uppercase tracking-tighter leading-[0.9] mb-3 group-hover:text-sky-600 transition-colors duration-300">
                                {edu.name}
                            </h3>
                            <p className="text-lg font-bold italic text-black/40 mb-6 uppercase tracking-tight">{edu.faculty}</p>
                            
                            <div className="flex items-center gap-6">
                                <div className="relative">
                                    <span className="text-xs font-black uppercase tracking-widest text-sky-600 block mb-1">GPAX</span>
                                    <div className="text-5xl md:text-6xl font-black tracking-tighter text-black leading-none">{edu.GPAX}</div>
                                </div>
                                <div className="h-12 w-px bg-black/10 mx-2" />
                                <div className="flex flex-col gap-1">
                                    <div className="flex gap-1">
                                        {[1, 2, 3, 4].map((star) => (
                                            <div key={star} className={`h-1.5 w-6 rounded-full ${star <= Math.floor(Number(edu.GPAX || 0)) ? 'bg-sky-500' : 'bg-black/10'}`} />
                                        ))}
                                    </div>
                                </div>
                            </div>
                            <div className="mt-8 w-32 h-1 bg-black/5 rounded-full overflow-hidden">
                                <motion.div initial={{ width: 0 }} whileInView={{ width: '100%' }} transition={{ duration: 1.5 }} className="h-full bg-sky-500" />
                            </div>
                        </div>
                    </motion.div>
                ))}
      ...
      <section id="skills" className="py-32 px-6 bg-[#fcfcfc]">
         <div className="max-w-7xl mx-auto">
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
             <div className="lg:col-span-8">
               <h2 className="text-4xl md:text-6xl font-black uppercase mb-16 flex items-center gap-4 text-zinc-900">
                 <Terminal className="text-sky-600" size={40} /> Tech <span className="text-sky-600">Universe.</span>
               </h2>
               <div className="space-y-16">
                {['Frontend', 'Backend', 'Database', 'AI & Machine Learning', 'Data Sciene', 'Tools'].map((categoryName) => {
                    
                    
                    const rawTechs = categorizedTech[categoryName];
                    ...
                    <div key={categoryName}>
                        <div className="mb-8">
                        <h4 className="text-[11px] font-black uppercase tracking-[0.5em] text-black mb-1 inline-block">
                            {categoryName}
                        </h4>
                        <div className="h-[3px] w-12 bg-gradient-to-r from-sky-500 to-transparent" />
                        </div>
                        
                        <div className="grid grid-cols-3 sm:grid-cols-5 gap-8">
                        {/* 3. ใช้ sortedTechs ที่เรียงแล้วมาแสดงผล */}
                        {sortedTechs.map((tech) => (
                            <div 
                            key={tech.id} 
                            className="group flex flex-col items-center gap-4 bg-white p-6 border-2 border-black/5 hover:border-black transition-all hover:shadow-[8px_8px_0px_#0ea5e9]"
                            >
                            <TechIcon logo={tech.logo} name={tech.name} />
                            <span className="text-[10px] font-black uppercase text-center opacity-40 group-hover:opacity-100 transition-opacity">
                                {tech.name}
                            </span>
                            </div>
                        ))}
            ...
            <div className="lg:col-span-4">
              <h2 className="text-4xl md:text-6xl font-black uppercase mb-16 text-zinc-900">Soft <span className="text-sky-600">Skills.</span></h2>
              <div className="space-y-4">
                {softSkills.map((skill) => (
                  <div key={skill.id} className="p-6 bg-white border-2 border-black flex items-center gap-4 hover:-translate-y-1 transition-transform shadow-[4px_4px_0px_black]">
                    <CheckCircle2 className="text-sky-600" />
                    <span className="font-black uppercase text-sm tracking-widest">{skill.name}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section id="hobbies" className="py-40 px-8 bg-[#121212] text-white overflow-hidden">
        <div className="max-w-7xl mx-auto">
            <h2 className="text-6xl md:text-9xl font-black uppercase tracking-tighter text-transparent transition-all duration-700 mb-20 cursor-pointer"
                style={{ 
                    WebkitTextStroke: '1px rgba(255,255,255,0.5)',
                    backgroundImage: 'linear-gradient(to right, #00f2fe 0%, #4facfe 100%)',
            ...
            <div className="flex flex-wrap justify-center gap-10">
                {hobbies.map((h, i) => (
                    <motion.div 
                        ...
                        className="bg-white p-4 pb-12 shadow-2xl rounded-sm w-72 h-96 group cursor-pointer"
                    >
                        <div className="relative w-full h-full bg-gray-200 overflow-hidden">
                            {h.image_url && <Image src={h.image_url} alt={h.name} fill className="object-cover grayscale group-hover:grayscale-0 transition-all duration-500" />}
                        </div>
                        <div className="mt-4 text-black font-black uppercase tracking-tighter text-center italic">{h.name}</div>
                    </motion.div>
                ))}
            </div>
        </div>
      ...
      <section id="projects" className="py-40 px-8 bg-[#fcfcfc]">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-32 border-b border-black pb-10">
             <h2 className="text-6xl font-black uppercase tracking-tighter text-zinc-900">My Project</h2>
             <span className="text-xs font-bold opacity-30 tracking-widest uppercase mb-2">(0{projects.length}) Total Projects</span>
          </div>
          
          <div className="space-y-40">
            {projects.map((p, idx) => (
              <div key={p.id} className="group cursor-pointer grid grid-cols-1 lg:grid-cols-12 gap-16 items-center" onClick={() => setSelectedProject(p)}>
                <div className="lg:col-span-7 relative aspect-video rounded-3xl overflow-hidden shadow-2xl bg-zinc-100">
                    {p.image_url && <Image src={p.image_url} alt={p.title} fill className="object-cover transition-transform duration-1000 group-hover:scale-105" />}
                    <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <div className="bg-white text-black px-8 py-3 rounded-full font-black text-xs uppercase tracking-widest shadow-xl">Explore Project</div>
                    </div>
                </div>
                <div className="lg:col-span-5 space-y-6">
                    <span className="text-sky-600 font-black text-sm uppercase italic">0{idx+1} — {p.position}</span>
                    <h3 className="text-5xl font-black uppercase tracking-tighter group-hover:text-sky-600 transition-colors duration-300">{p.title}</h3>
                    <p className="text-xl text-black/40 font-medium leading-relaxed italic">&quot;{p.content}&quot;</p>
                    <div className="flex gap-4 items-center pt-6">
                        <span className="w-12 h-px bg-black" />
                        <span className="text-[10px] font-black uppercase tracking-[0.3em]">{p.company}</span>
                    </div>
                </div>
              </div>
            ))}
       ...
       <section id="contact" className="py-32 px-6 bg-[#f4f4f4]">
         <div className="max-w-4xl mx-auto text-center">
           <h2 className="text-5xl md:text-8xl font-black uppercase mb-16 tracking-tighter text-zinc-900 italic">Let&apos;s Contact <br/><span className="bg-sky-500 text-white px-4 not-italic">Together</span></h2>
          
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
             <a href={`mailto:${contracts?.gmail}`} className="bg-white border-2 border-black p-10 hover:shadow-[10px_10px_0px_black] transition-all group">
               <Mail className="mx-auto mb-4 group-hover:text-sky-600 transition-colors" size={40} />
               <div className="font-black uppercase text-xl truncate">{contracts?.gmail}</div>
             </a>
             <a href={`tel:${contracts?.phone}`} className="bg-white border-2 border-black p-10 hover:shadow-[10px_10px_0px_black] transition-all group">
               <Phone className="mx-auto mb-4 group-hover:text-sky-600 transition-colors" size={40} />
               <div className="font-black uppercase text-xl">{contracts?.phone}</div>
             </a>
           </div>

           <div className="flex justify-center gap-12">
             <a href={contracts?.github || "#"} target="_blank" className="hover:text-sky-600 transition-all hover:scale-125"><Github size={32} /></a>
             <a href={contracts?.linkedIn || "#"} target="_blank" className="hover:text-sky-600 transition-all hover:scale-125"><Linkedin size={32} /></a>
             <a href="https://line.me/ti/p/~tawatchai23987" target="_blank" className="hover:text-sky-600 transition-all hover:scale-125"><MessageCircle size={32} /></a>
           </div>
         </div>
      </section>
      ...
      <AnimatePresence>
        {selectedProject && (
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25 }} className="fixed inset-0 z-[200] bg-white overflow-y-auto">
            <div className="max-w-4xl mx-auto px-8 py-0 pb-10 relative">
              <button onClick={() => setSelectedProject(null)} className="fixed top-10 right-10 z-[210] flex items-center gap-4 text-xs font-black uppercase tracking-widest bg-black text-white px-6 py-3 rounded-full hover:bg-sky-600 transition-colors">
                <X size={16} /> Close
              </button>
              
              <div className="mt-20">
                <div className="text-sky-600 font-black text-xs uppercase tracking-widest mb-6 italic">{selectedProject.company} {selectedProject.position}</div>
                <h2 className="text-6xl md:text-8xl font-black uppercase tracking-tighter mb-12 leading-none">{selectedProject.title}</h2>
                
                <div className="relative aspect-video rounded-3xl overflow-hidden mb-20 shadow-2xl bg-zinc-100">
                    {selectedProject.image_url && <Image src={selectedProject.image_url} alt="detail" fill className="object-cover" />}
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-20">
                    <div className="md:col-span-2 space-y-12">
                        <section>
                            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] mb-6 opacity-30 underline decoration-sky-500 underline-offset-4">Overview</h4>
                            <p className="text-2xl md:text-3xl font-light italic text-black/60 leading-relaxed">&quot;{selectedProject.content}&quot;</p>
                        </section>
                        <section>
                            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] mb-6 opacity-30 underline decoration-sky-500 underline-offset-4">Project Tasks</h4>
                            <p className="text-lg leading-relaxed whitespace-pre-line text-black/80 font-medium pl-8 border-l-4 border-black">{selectedProject.tasks}</p>
                        </section>
                    </div>
                    <div className="bg-[#f9f9f9] p-10 rounded-3xl h-fit border border-black/5">
                        <div className="space-y-8">
                            <div>
                                <div className="text-[8px] font-black uppercase text-black/30 tracking-widest mb-1">Status</div>
                                <div className="text-sm font-black uppercase italic">Completed</div>
                            </div>
                            <div>
                                <div className="text-[8px] font-black uppercase text-black/30 tracking-widest mb-1">Type</div>
                                <div className="text-sm font-black uppercase italic text-sky-600">{selectedProject.employmentType}</div>
                            </div>
                        </div>
                    </div>
                </div>
